package com.ccunix.ihousekeeping.minprog_main.domain;

import com.ccunix.ihousekeeping.basedb.domain.T_u_coupon;

public class Coupon extends T_u_coupon{
	private String type_name;
	private String back_pic;
	private String coupon_project_name;
	private Integer true_money;
	public Integer getTrue_money() {
		return true_money;
	}
	public void setTrue_money(Integer true_money) {
		this.true_money = true_money;
	}
	public String getType_name() {
		return type_name;
	}
	public void setType_name(String type_name) {
		this.type_name = type_name;
	}
	public String getBack_pic() {
		return back_pic;
	}
	public void setBack_pic(String back_pic) {
		this.back_pic = back_pic;
	}
	public String getCoupon_project_name() {
		return coupon_project_name;
	}
	public void setCoupon_project_name(String coupon_project_name) {
		this.coupon_project_name = coupon_project_name;
	}
}
