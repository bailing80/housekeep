...
//批量删除 
	...
	public AppDataModel getHeadSortFirst(Base_Model_Business base_Model_Business) throws Exception {
			...
			for (int i = 0; i < service_project_name.size(); i++) {
				...
				t.setTop_picture(paramManager_Service_Iface.getSystemParam(SystemParam.SYSPARM.httpFileBasePath)+t.getTop_picture());
				System.out.println(t.getTop_picture()+"#######");
			}
			List service_item1=serviceproject_Manamger_Dao_Iface.query1();
//			T_u_service_item t_u_service_item=new T_u_service_item();
//			List service_item = t_u_service_item_Dao_Iface.queryAll_t_u_service_item(t_u_service_item);
			appDataModel.data.put("service_project", service_project_name);
			appDataModel.data.put("advertisement", advertisement);
			appDataModel.data.put("service_item", service_item1);
			System.out.println(appDataModel.data.size()+"@@@@@@@@@@");
			return appDataModel;
		}
