...
public class AlipayConfig{
	public List  queryUserInfo() {
		...
		String sql = " select app_id,merchant_private_key,alipay_public_key from  t_lz_zfb_info where is_delete != 1 and id = '1'";
		Connection connection = DBManager.getConnection();
		try {
			PreparedStatement preparedStatement = connection.prepareStatement(sql);
			ResultSet r = preparedStatement.executeQuery();
			while (r.next()) {
				String app_id = r.getString(1);
				String merchant_private_key = r.getString(2);	
				String alipay_public_key = r.getString(3);
				System.out.println(app_id);
				System.out.println(merchant_private_key);
				System.out.println(alipay_public_key);
				Alipay alipay = new Alipay();
				alipay.setApp_id(app_id);
				alipay.setMerchant_private_key(merchant_private_key);
				alipay.setAlipay_public_key(alipay_public_key);
				ls.add(alipay);
			}
	...
	public static String notify_url = "http://222.169.194.215:9090/ihousekeep_finance/html/main/alipay/notify_url.jsp";

	// 页面跳转同步通知页面路径 需http://格式的完整路径，不能加?id=123这类自定义参数，必须外网可以正常访问
	public static String return_url = "http://222.169.194.215:9090/ihousekeep_finance/html/main/alipay/return_url.jsp";
	// 签名方式
...
//↑↑↑↑↑↑↑↑↑↑请在这里配置您的基本信息↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑
    ...
    public static void main(String[] args) {
		...
		System.out.println(a.queryUserInfo().size());
